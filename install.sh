...
# Expose handy bin scripts on $PATH (~/.local/bin is on PATH per zsh/env.zsh).
link "$DOTFILES/bin/copilot-usage"      "$HOME/.local/bin/copilot-usage"

mkdir -p "$HOME/.ssh/config.d"
chmod 700 "$HOME/.ssh" "$HOME/.ssh/config.d"

# Initialise age key + fnox global config so `fnox set --global FOO bar`
# works in any shell straight after install. Both steps are idempotent.
# bin/bootstrap-secrets later overwrites these with values from your vault.
AGE_KEY_FILE="$HOME/.config/age/keys.txt"
FNOX_GLOBAL_CONFIG="$HOME/.config/fnox/config.toml"

if [[ ! -f "$AGE_KEY_FILE" ]]; then
  log "Generating age key at $AGE_KEY_FILE"
  mkdir -p "$HOME/.config/age"
  ( umask 077 && /usr/bin/age-keygen -o "$AGE_KEY_FILE" >/dev/null )
fi

if [[ ! -f "$FNOX_GLOBAL_CONFIG" ]]; then
  age_pub="$(grep -m1 '# public key:' "$AGE_KEY_FILE" | awk '{print $NF}')"
  if [[ -n "$age_pub" ]]; then
    log "Initialising fnox global config at $FNOX_GLOBAL_CONFIG"
    mkdir -p "$HOME/.config/fnox"
    umask 077
    cat > "$FNOX_GLOBAL_CONFIG" <<EOF
# fnox global config — auto-loaded into every shell by \`fnox activate zsh\`.
# Decryption uses \$FNOX_AGE_KEY_FILE (exported by zsh/env.zsh).
...
recipients = ["$age_pub"]
EOF
    chmod 600 "$FNOX_GLOBAL_CONFIG"
  else
    warn "Could not parse age public key from $AGE_KEY_FILE; skipping fnox init"
  fi
fi
